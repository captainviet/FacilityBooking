...
public class Server {
    ...
    private void logRequest(String requestId, String requestType, String clientIp, int clientPort) {
    	System.out.printf("[%s] Successfully processed request, id: %s, type: %s, client %s:%d", Utils.currentLogFormatTime(),
    			requestId, requestType, clientIp, clientPort);
	}
	...
	private String processGetAllAvailableInTimeRangeRequest(Request request) {
    	...
    	if (mode == SemanticsMode.AT_MOST_ONCE) {
     	...
     	logRequest(request.getId(), request.getType(), serverSocket.getClientHost().getHostAddress(), serverSocket.getClientPort());
    	return null;
	...
	private String processMonitorRequest(Request request) {
		...
		if (facility == null) {
			...
			ClientMonitor clientMonitor = new ClientMonitor(serverSocket.getClientHost(), serverSocket.getClientPort(), facilityName, endMonitorDateTime);
